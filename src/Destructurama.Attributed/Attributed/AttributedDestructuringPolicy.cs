...
namespace Destructurama.Attributed
{
    class AttributedDestructuringPolicy : IDestructuringPolicy
    {
        ...
        {
            var classDestructurer = type.GetTypeInfo().GetCustomAttribute<IClassDestructurer>();
            ...
            if (properties.All(pi => pi.GetCustomAttribute<IPropertyDestructurer>() == null))
                return CacheEntry.Ignore;
            
            var destructuringAttributes = properties
                .Select(pi => new {pi, Attribute = pi.GetCustomAttribute<IPropertyDestructurer>() })
        ...
        static LogEventPropertyValue MakeStructure(object o, IEnumerable<PropertyInfo> loggedProperties, IDictionary<PropertyInfo, IPropertyDestructurer> destructuringAttributes, ILogEventPropertyValueFactory propertyValueFactory, Type type)
        {
